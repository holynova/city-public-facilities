...
const CITY_RAIL_CONFIG = {
  ...
  "深圳市": {
    cityCode: "0755",
    lineQueries: [...Array.from({ length: 12 }, (_, index) => `地铁${index + 1}号线`), "地铁14号线", "地铁16号线", "地铁20号线", "地铁6号线支线"],
    outputFile: "amap-metro-lines.json",
  },
  "苏州市": {
    cityCode: "0512",
  ...
  "重庆市": {
    cityCode: "023",
    lineQueries: [...Array.from({ length: 6 }, (_, index) => `地铁${index + 1}号线`), "地铁9号线", "地铁10号线", "地铁18号线", "重庆轨道交通环线", "环线", "江跳线", "璧山线"],
    outputFile: "amap-metro-lines.json",
  },
  "武汉市": {
    cityCode: "027",
    lineQueries: [...Array.from({ length: 8 }, (_, index) => `地铁${index + 1}号线`), "地铁11号线", "地铁16号线", "地铁19号线", "阳逻线"],
    outputFile: "amap-metro-lines.json",
  },
  "西安市": {
    cityCode: "029",
    lineQueries: [...Array.from({ length: 6 }, (_, index) => `地铁${index + 1}号线`), "地铁9号线", "地铁10号线", "地铁14号线", "地铁16号线", "地铁17号线", "地铁18号线", "地铁19号线", "机场城际线"],
    outputFile: "amap-metro-lines.json",
  },
  "芜湖市": {
    cityCode: "0553",
...
export async function collectMetroFromLines(snapshot: string, city = "上海市"): Promise<AmapCollectedFacilityRecord[]> {
  ...
  const client = new AmapClient(city === "成都市" ? 6_500 : ["重庆市", "武汉市", "西安市"].includes(city) ? 2_000 : 1_100);
